...
To send a one-off trace to an explicit path instead, set `VPN_SWITCH_TRACE_FILE` (the path
must contain a directory component). The whole recursive chain lands in that one file,
...
One **top-level invocation** produces one `.log`/`.trace` pair, named after the function the
command resolved to (`vpn-switch wireguard list` → `…__wireguard_list0.trace`). Nested
dispatches (combinator rewrites, batch steps) inherit `VPN_SWITCH_TRACE_FILE` and append to
the **same** trace file, indented by call depth — you get the whole rewrite chain in one file.
The PID in the filename is the vpn-switch process, **not** a VPN session PID; to find the trace
for a command you ran, match the function name and timestamp, not the session.

### Anatomy of a trace

A real (abridged) trace of `vpn-switch wireguard list`:

```
| process_arguments: provided arguments 'wireguard list' resolve to function call: '_wireguard_list0'
| process_arguments: Interpreter resolved: cut -b3-
| produce_with_exit: ENTRY: VPN_SWITCH_CONTEXT_CALL='_wireguard_list0'
> run_env: External command: cut -b3-
= process_arguments: --- input to interpreter ---
# === WireGuard Configurations ===
# ...
< run_env: External command completed (exit: 0)
> process_arguments: --- output from interpreter ---
=== WireGuard Configurations ===
...
| process_arguments: producer exit code: 0
| process_arguments: consumer exit code: 0
| process_arguments: combined exit code: 0
```

Entry format: `<depth><marker> <caller>: <message>`, where depth is `>  ` repeated per
nesting level and the markers are:

| Marker | Meaning |
|--------|---------|
| `>` | subprocess entry (before an interpreter/`run_env` call) |
| `<` | subprocess exit |
| `\|` | informational message |
| `=` | separator |

The five lines that answer most questions:

- `resolve to function call:` — which function the arguments dispatched to.
- `Interpreter resolved:` — which interpreter actually ran (the usual suspect: `cat` when
  you expected `sh`, or vice versa).
- `--- input to interpreter ---` — the exact text the function emitted (the generated
  script/plan, before the interpreter touches it).
- `--- output from interpreter ---` — what came out the other side.
- `producer/consumer/combined exit code:` — where a failure entered the pipeline: producer
  = the function itself, consumer = the interpreter run.

### Worked example: "the command prints nothing / doesn't execute"

```bash
vpn-switch session list        # produces no output — why?
f=$(ls -t "$VPN_SWITCH_BASE/.log/$(date +%Y-%m-%d)/"*session_list*.trace | head -1)
grep -e 'Interpreter resolved' -e 'input to interpreter' "$f"
```

If `input to interpreter` shows the expected `# `-prefixed data but the interpreter is `sh`,
the data lines were swallowed as comments — the interpreter is misconfigured (should be
`cut -b3-`). If the input section is empty, the function itself produced nothing: look above
it for the `resolve to function call:` line and dispatch/argument problems.
